...
for i in $(seq 1 15); do
    if curl -s -o /dev/null http://localhost/api/temp; then
        echo "✅ Servidor llest!"
        break
    fi
    sleep 1
...
xdg-open http://localhost 2>/dev/null \
    || google-chrome --app=http://localhost 2>/dev/null \
    || firefox http://localhost 2>/dev/null \
    || echo "No s'ha trobat cap navegador. Obre manualment: http://localhost"
